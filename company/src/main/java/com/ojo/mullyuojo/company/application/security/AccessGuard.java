...
public class AccessGuard {
        ...
        switch(ctx.getRole()){
            ...
            case COMPANY_MANAGER -> {
                ...
                if(action == Action.DELETE){
                    throw new BusinessException(ErrorCode.ACCESS_DENIED, "엄체 담당자는 삭제 권한이 없습니다.");
                }
                if(action == Action.CREATE){
                    throw new BusinessException(ErrorCode.ACCESS_DENIED, "엄체 담당자는 생성 권한이 없습니다.");
                }
            }
        }
    ...
    private static void requireCompany(ResourceScope scope){
        if (scope == null || scope.getCompanyId() == null){
